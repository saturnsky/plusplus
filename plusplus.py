...
class PlusPlus(object):
    ...
    @classmethod
    def find_hanja(cls, src):
        start, end, position = -1, -1, -1
        mode = 'hanja_normal'
        for char in src:
            position += 1
            if char in cls.hanja_normal or char in cls.hanja_difficult:
                if end == position:
                    end = position + 1
                else:
                    start, end = position, position + 1
        for char in src[start:end]:
            if char in cls.hanja_difficult:
    ...
    @classmethod
    def find_roma(cls, src):
        start, end, position = -1, -1, -1
        mode = 'roma_normal'
        for char in src:
            position += 1
            if ('a' <= char <= 'z' or 'A' <= char <= 'Z') \
                    and char not in cls.roma_normal \
                    and char not in cls.roma_capital_normal:
                start, end, mode = -1, -1, None
                break
            if char in cls.roma_normal or char in cls.roma_capital_normal:
                if char in cls.roma_capital_normal:
        ...
        for char in src:
            position += 1
            if char in cls.roma_special and start < position:
                start, end, mode = position, position + 1, 'roma_special'
            if char in cls.roma_capital_special and start < position:
    ...
    @classmethod
    def find_value(cls, src):
        result_hanja = cls.find_hanja(src)
        result_roma = cls.find_roma(src)
        result_arabia = cls.find_arabia(src)
        result = (-1, -1, None)
    ...
    @classmethod
    def process_value(cls, src, mode):
        result = src
    ...
    def plusone(cls, raw_src):
        ...
        if result[2] != None:
            processed_value = cls.process_value(src[result[0]:result[1]], result[2])
